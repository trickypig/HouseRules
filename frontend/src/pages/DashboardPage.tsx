...
export default function DashboardPage() {
  ...
  const [quickKidId, setQuickKidId] = useState<number | null>(null);
  ...
  async function handleQuickTransaction(e: FormEvent) {
    ...
    try {
      await api.createTransaction(quickKidId, {
        type: quickType,
  ...
  async function handleProcessAllowances() {
    setProcessing(true);
    setProcessResult('');
    try {
      const result = await api.processAllowances();
      if (result.count === 0) {
        setProcessResult('No allowances due today.');
      } else {
        const names = result.processed.map(p => `${p.kid_name} ($${p.amount.toFixed(2)})`).join(', ');
        setProcessResult(`Processed ${result.count} allowance(s): ${names}`);
      }
      loadDashboard();
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : 'Failed to process allowances');
    } finally {
      setProcessing(false);
    }
  }
    ...
    <div>
      <div className="section-header">
        ...
        <div style={{ display: 'flex', gap: '0.5rem' }}>
          <button onClick={handleProcessAllowances} className="btn btn-outline" disabled={processing}>
            {processing ? 'Processing...' : 'Process Allowances'}
          </button>
      ...
      {processResult && <div className="success-message">{processResult}</div>}
              ...
              <Link to={`/kids/${kid.id}`} key={kid.id} className="list-card">
                ...
                {kid.allowance && (
                  <div className="list-card-meta">
                    <span>Allowance: {formatCurrency(kid.allowance.amount)} / {kid.allowance.frequency}</span>
                  </div>
                )}
            ...
            <form onSubmit={handleQuickTransaction}>
              <div className="form-row">
                <div className="form-group form-group-grow">
                  ...
                  <select id="qKid" value={quickKidId ?? ''} onChange={e => setQuickKidId(e.target.value ? Number(e.target.value) : null)} required>
                  ...
                  <div key={tx.id} className="item-card">
                    <div className="item-details">
                      <div className="item-header">
                        ...
                        <span style={{ fontSize: '0.8rem', color: 'var(--color-text-muted)' }}>{tx.category}</span>
                      </div>
